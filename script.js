(() => {
  ...
  function renderData() {
    ...
    $("#academics-detail").innerHTML = `<div class="academic-group academic-scores"><h3>coursework / scores</h3><dl>${data.academics.coursework.map((entry) => { const split = entry.lastIndexOf(" — "); return `<div><dt>${escapeHTML(entry.slice(0, split))}</dt><dd>${escapeHTML(entry.slice(split + 3))}</dd></div>`; }).join("")}</dl></div><div class="academic-group academic-activities"><h3>activities</h3>${data.academics.activities.map((entry) => { const split = entry.indexOf(" — "); return `<div class="academic-activity"><strong>${escapeHTML(entry.slice(0, split))}</strong><p>${escapeHTML(entry.slice(split + 3))}</p></div>`; }).join("")}</div><div class="academic-group academic-distinctions"><h3>distinctions</h3>${data.academics.distinctions.map((entry) => `<p>${escapeHTML(entry)}</p>`).join("")}</div>`;
    $("#plans-list").innerHTML = data.plans.map(({ heading, paragraphs, closing }, index) => `<details class="plan-entry"><summary><span class="plan-title">${escapeHTML(heading)}</span></summary><div class="plan-prose">${paragraphs.map((paragraph, paragraphIndex) => {
